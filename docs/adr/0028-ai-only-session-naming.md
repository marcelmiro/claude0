...
**Generation names the subject.** Context sent to `claude -p --model haiku`: plan title, first user message, **first assistant reply**, summary, last user message, **last assistant reply**, branch (assistant snippets read lazily at naming time via `readNamingExtras` — never on discovery sweeps). The prompt demands the subject of the work, never the interaction mode alone. Refusal-shaped output is salvaged (strip one conversational opener, clamp to 4 words) before rejection; hard refusals ("I can't…") are never salvaged.
