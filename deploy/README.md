...
| `../config/units/claude0-monitor.service` | User unit: fallback monitor tick while no tmux client is attached (status-right only runs for attached clients) + unconditional 5-min resurrect autosave — the host's only periodic saver. |
