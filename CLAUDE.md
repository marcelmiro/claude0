...
- **When it is NOT needed:** changes to `src/bridge/public/*` (`app.js`, `index.html`/CSS). Those are served fresh (`cache-control: no-cache`); the user just refreshes/reopens the page on the phone. The service worker also keeps a network-first shell cache (`claude0-shell-v1`) as an offline fallback for the tunnel-wake window — a reachable bridge always wins, so refreshes still serve fresh code ([ADR 21](docs/adr/0021-offline-app-shell.md)).
